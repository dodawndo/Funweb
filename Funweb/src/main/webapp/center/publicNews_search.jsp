...
int listCount = boardDAO2.selectSearchListCount(search, searchType);
...
ArrayList<BoardBean2> boardList2 = boardDAO2.selectSearchBoardList(pageNum, listLimit, search, searchType);
...
<head>
<meta charset="UTF-8">
<title>center/publicNews.jsp</title>
...
// 				}
				...
				%>
					...
					<tr onclick="location.href='publicNews_content.jsp?num=<%=board2.getNum() %>&page=<%=pageNum %>'">
						<td><%=board2.getNum() %></td>
						<td class="left"><%=board2.getTitle() %></td>
			...
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" onclick="location.href='publicNews_write.jsp'">
			</div>
			<div id="table_search">
				<form action="publicNews_search.jsp" method="get">
					<select name="searchType">
						...
						<option value="writer" <%if(searchType.equals("writer")) { %>selected="selected" <%} %>>글쓴이</option>
					</select>
			...
			<div id="page_control">
				...
				<%if(pageNum > 1) { // 이전페이지가 존재할 경우 %>
					<a href="publicNews_search.jsp?page=<%=pageNum - 1%>&search=<%=search%>&searchType=<%=searchType%>">Prev</a>
				...
				<%for(int i = startPage; i <= endPage; i++) { %>
					...
					<%if(pageNum == i) { %>
						&nbsp;&nbsp;<%=i %>&nbsp;&nbsp;
					<%} else { %>
						<a href="publicNews_search.jsp?page=<%=i%>&search=<%=search%>&searchType=<%=searchType%>"><%=i %></a>
					<%} %>
				<%} %>
				<!-- 
				...
				<%if(pageNum < maxPage) { // 다음페이지가 존재할 경우 %>
					<a href="publicNews_search.jsp?page=<%=pageNum + 1%>&search=<%=search%>&searchType=<%=searchType%>">Next</a>
				<%} else { // 이전페이지가 존재하지 않을 경우 %>
					&nbsp;Next
				<%} %>
			</div>
